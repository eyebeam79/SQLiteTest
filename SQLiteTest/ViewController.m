...
//  SQLiteTest
//
//  Created by Jinho Son on 2014. 1. 12..
...
- (void)openDB
{
    ...
    NSAssert1(ret == SQLITE_OK, @"Error on opening Database: %s", sqlite3_errmsg(db));
    ...
    if (existFile == NO)
    {
        ...
        if (ret != SQLITE_OK)
        {
            ...
            NSAssert1(ret == SQLITE_OK, @"Error on creating table: %s", errorMsg);
...
- (void)resolveData
{
    ...
    NSAssert2(ret == SQLITE_OK, @"Error(%d) on resolving data: %s", ret, sqlite3_errmsg(db));
